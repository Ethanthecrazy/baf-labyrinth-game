...
namespace AnimationEditor
{
    public partial class Form1 : Form
    {
        ...
        private void DrawInfoFramePanel(Point offset) 
        {
            ...
            {
                //Draw Rect
                DrawRect(currAnim.frames[i].RDrawRect, offset, Color.Green);
                //Collison Rect
                DrawRect(currAnim.frames[i].RCollisionRect, offset, Color.Red);
                //Anchor Point
                int x = currAnim.frames[i].PAnchor.X;
                int y = currAnim.frames[i].PAnchor.Y;
                dev.DrawRect(new Rectangle(x - offset.X, y - offset.Y, 5, 5), 255, 255, 0);
            }
        }
